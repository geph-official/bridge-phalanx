...
pub struct ScalewayBaremetalProvider {
    cfg: ScalewayConfig,
}

impl ScalewayBaremetalProvider {
...
impl Provider for ScalewayBaremetalProvider {
    fn create_server(&self, id: &str) -> smol::Task<anyhow::Result<String>> {
        ...
        smol::spawn(async move {
            let resp = Request::post(format!(
                "https://api.scaleway.com/baremetal/v1/zones/{}/servers",
                cfg.zone
            ))
            ...
            .body(serde_json::to_vec(&CreateServerReq {
                ...
                offer_id: cfg.commercial_type.clone(),
...
async fn get_server(cfg: &ScalewayConfig, scw_server_id: &str) -> anyhow::Result<ServerResp> {
    let mut response = Request::get(format!(
        "https://api.scaleway.com/baremetal/v1/zones/{}/servers/{scw_server_id}",
        cfg.zone
    ))
    .header("X-Auth-Token", &cfg.secret_key)
    .body("")?
...
async fn perform_action(
    ...
    let request = Request::post(format!(
        "https://api.scaleway.com/baremetal/v1/zones/{}/servers/{}/action",
